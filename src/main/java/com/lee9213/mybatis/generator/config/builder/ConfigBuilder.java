...
public class ConfigBuilder {
     ...
     * 模板路径配置信息
     */
    private final TemplateConfiguration template;
    /**
     * 数据库配置
     */
     ...
     */
    public ConfigBuilder(GlobalConfiguration globalConfig, TemplateConfiguration template,
                         PackageConfiguration packageConfig, DataSourceConfiguration dataSourceConfig,
        ...
        if (null == packageConfig) {
            handlerPackage(this.template, this.globalConfig.getOutputDir(), new PackageConfiguration());
        } else {
        ...
        if (null == strategyConfig) {
            this.strategyConfig = new StrategyConfiguration();
        } else {
     ...
     */
    private void handlerPackage(TemplateConfiguration template, String outputDir, PackageConfiguration config) {
        // 包信息
        packageInfo = new HashMap<>(6);
        packageInfo.put(Constant.GENERATOR, "");
        packageInfo.put(Constant.MODULE_NAME, config.getModuleName());
        packageInfo.put(Constant.ENTITY, joinPackage(config.getParent(), config.getEntity()));
        packageInfo.put(Constant.MAPPER, joinPackage(config.getParent(), config.getMapper()));
        packageInfo.put(Constant.XML, joinPackage(config.getParent(), config.getXml()));
        packageInfo.put(Constant.SERVICE, joinPackage(config.getParent(), config.getService()));
        packageInfo.put(Constant.SERVICE_IMPL, joinPackage(config.getParent(), config.getServiceImpl()));
        packageInfo.put(Constant.CONTROLLER, joinPackage(config.getParent(), config.getController()));
        ...
        if (null != configPathInfo) {
            ...
            // TODO 优化路径
            setPathInfo(pathInfo, template.getGenerator(), outputDir+"\\src\\main\\resources", Constant.GENERATOR_PATH, Constant.GENERATOR);
            setPathInfo(pathInfo, template.getEntity(), outputDir+"\\src\\main\\java", Constant.ENTITY_PATH, Constant.ENTITY);
            setPathInfo(pathInfo, template.getMapper(), outputDir+"\\src\\main\\java", Constant.MAPPER_PATH, Constant.MAPPER);
            setPathInfo(pathInfo, template.getXml(), outputDir+"\\src\\main\\resources", Constant.XML_PATH, Constant.XML);
            setPathInfo(pathInfo, template.getService(), outputDir+"\\src\\main\\java", Constant.SERVICE_PATH, Constant.SERVICE);
            setPathInfo(pathInfo, template.getServiceImpl(), outputDir+"\\src\\main\\java", Constant.SERVICE_IMPL_PATH, Constant.SERVICE_IMPL);
            setPathInfo(pathInfo, template.getController(), outputDir+"\\src\\main\\java", Constant.CONTROLLER_PATH, Constant.CONTROLLER);
        }
    }
     ...
     */
    private void handlerDataSource(DataSourceConfiguration config) {
     ...
     */
    private void handlerStrategy(StrategyConfiguration config) {
        processTypes(config);
     ...
     * </p>
     */
    private List<TableInfo> getTablesInfo(StrategyConfiguration config) {
...
//            includeTableList.forEach(ti -> convertTableFields(ti, config.getColumnNaming()));
     ...
     * @return 补充完整信息后的表
     */
    private List<TableInfo> processTable(List<TableInfo> tableList, NamingStrategy strategy, StrategyConfiguration config) {
